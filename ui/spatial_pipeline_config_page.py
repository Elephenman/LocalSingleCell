...
def show():
    ...
    """
    # 页面前置校验
    if not st.session_state.is_data_loaded or not st.session_state.get('is_spatial_data', False):
        st.warning("请先导入空间转录组数据")
        return
    
    st.title("空间转录组分析流程配置")
        ...
        if st.button("保存参数配置"):
            ...
            with open(tmp_path, 'rb') as f:
                st.download_button(
                    label="下载配置文件",
                    data=f,
                    file_name="spatial_pipeline_config.yaml",
    ...
    with st.expander("步骤1：质控与过滤", expanded=True):
        ...
        # 预览质控结果按钮
        if st.button("预览质控结果", key="spatial_qc_preview"):
            try:
                # 复制AnnData对象进行临时计算
    ...
    # 步骤5：空间专属分析
    with st.expander("步骤5：空间专属分析", expanded=True):
        st.subheader("空间专属分析参数")
        
        # 空间邻居图构建
        st.markdown("#### 空间邻居图构建")
        coord_type = st.selectbox(
            "坐标类型",
            ["grid（网格型，如Visium）", "generic（通用型）"],
            index=0 if config['spatial']['coord_type'] == 'grid' else 1
        )
        config['spatial']['coord_type'] = 'grid' if coord_type.startswith('grid') else 'generic'
        
        n_rings = st.slider("邻居环数", min_value=1, max_value=5, step=1, value=config['spatial']['n_rings'])
        config['spatial']['n_rings'] = n_rings
        
        delaunay = st.checkbox("使用Delaunay三角剖分", value=config['spatial']['delaunay'])
        config['spatial']['delaunay'] = delaunay
        
        # 空间可变基因分析
        st.markdown("#### 空间可变基因分析")
        svg_apply = st.checkbox("执行空间可变基因分析", value=config['spatial']['spatial_variable_genes']['apply'])
        if svg_apply:
            svg_mode = st.selectbox(
                "自相关模式",
                ["Moran's I", "Geary's C"],
                index=0 if config['spatial']['spatial_variable_genes']['mode'] == 'moran' else 1
            )
            config['spatial']['spatial_variable_genes']['mode'] = 'moran' if svg_mode == "Moran's I" else 'geary'
        config['spatial']['spatial_variable_genes']['apply'] = svg_apply
        
        # 共定位分析
        st.markdown("#### 共定位分析")
        colocalization_apply = st.checkbox("执行细胞类型共定位分析", value=config['spatial']['colocalization']['apply'])
        if colocalization_apply:
            n_splits = st.slider("分割数", min_value=1, max_value=10, step=1, value=config['spatial']['colocalization']['n_splits'])
            config['spatial']['colocalization']['n_splits'] = n_splits
        config['spatial']['colocalization']['apply'] = colocalization_apply
    
    # 步骤6：差异基因分析
    with st.expander("步骤6：差异基因分析"):
    ...
    # 保存配置并执行分析按钮
    if st.button("保存配置并执行空间分析"):
        try:
            # 更新全局状态
            st.session_state.pipeline_config = config
            
            # 执行分析
            from core import spatial_pipeline
            
            with st.spinner("正在执行空间转录组分析..."):
                adata, result = spatial_pipeline.run_spatial_pipeline(
